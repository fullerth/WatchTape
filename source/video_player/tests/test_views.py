...
class VideoTaggingTests(TestCase):
    ...
    def _add_bout(self, video, v_to_j_time='0h0m2s'):
        ...
        video_to_jam = VideoToJam.objects.create(video=video,
                                                 jam=jam)
        video_to_jam.start_time = v_to_j_time
        video_to_jam.save()

        return{'home_roster':home_roster, 'away_roster':away_roster, 'jam':jam, 'video_to_jam':video_to_jam}
    ...
    def test_video_player_context_contains_times_in_one_bout(self):
        bout_info = self._add_bout(self.video)
        response = self.client.get('/')
        time_list = map(repr, [bout_info['video_to_jam'].start_seconds])
        self.assertQuerysetEqual(response.context['times'], time_list)


    def test_video_player_context_contains_times_in_two_bouts(self):
        bout_1_info = self._add_bout(self.video, v_to_j_time='1h5m18s')
        bout_2_info = self._add_bout(self.video, v_to_j_time='2h2m18s')
        response = self.client.get('/')
        time_list = map(repr, [bout_1_info['video_to_jam'].start_seconds,
            bout_2_info['video_to_jam'].start_seconds])
        self.assertQuerysetEqual(response.context['times'], time_list)
